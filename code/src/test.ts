...
async function main() {
    ...
    r = await axios.patch(cfg.baseUrl + '/api/themes/def', {
        themedata: '4567',
        color: { primary: 'green' },
    });
    ...
    r = await axios.patch(cfg.baseUrl + '/api/themes/def?returnDoc=true', {
        $set: { 'color.primary': 'brown', 'color.secondary': 'black' },
    });
    console.log('updateDoc =', r.data);
    myAssert(
        r.data && r.data.themedata == '4567' && r.data.color.primary == 'brown',
        'updateDoc def failed'
    );
    //myAssert((r.data == true), "updateDoc def failed");
    ...
    myAssert(
        r.data &&
            r.data.color.primary == 'brown' &&
            r.data.color.secondary == 'black',
        'getDoc def failed'
    );
    ...
    myAssert(
        r.data &&
            r.data.color.primary == 'brown' &&
            r.data.color.secondary == undefined,
        'getDoc def failed'
    );
    ...
    console.log('deleteDoc =', r.data);
    myAssert(
        r.data.id == 'def' && r.data.themedata == '4567',
        'deleteDoc failed'
    );
    ...
    myAssert(
        r.data.indexOf('You need to enable JavaScript to run this app') > -1,
        'Failed getting HTML'
    );
