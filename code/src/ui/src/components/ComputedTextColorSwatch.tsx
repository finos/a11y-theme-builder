...
export const ComputedTextColorSwatch: React.FC<Props> = ({
    sampleText,
    colorSampleClassName,
    textSampleClassName,
    label,
    style,
    containerClassName,
    ...
    useEffect(() => {
        ...
        if (!showHex) {
            const colorRGBA = processColorIntoRGBA(
                cssObj.color,
                cssObj.opacity
            );
    ...
    if (sampleText) {
        return (
            <div
                className={`color-swatch ${containerClassName || ''} text-swatch`}
            >
                {label && <div className="small-semibold">{label}</div>}
                <div
                    className="color-box"
                    style={{ backgroundColor: `${_backgroundColorRGB}` }}
                />
                <div>
                    <div className="small-semibold">Sample: </div>
                    <div
                        className={`color-title ${textSampleClassName || ''}`}
                        ref={colorTextRef}
                    >
                        {sampleText}
                    </div>
