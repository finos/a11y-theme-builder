...
export const TextEditBox: React.FC<Props> = ({
  designSystem,
  cssPrefix,
  textKey,
  isBody = false,
    ...
    <>
      ...
      <Grid container spacing={2}>
        ...
        <Grid item lg={6} md={4} sm={12}>
          <div className="label-1">Styling</div>
          <ul className="styling caption quiet">
            <li className="font-family">
              font-family:{' '}
              <span>{styles && styles.getPropertyValue('font-family')}</span>
            </li>
            <li className="font-weight">
              font-weight:{' '}
              <span>{styles && styles.getPropertyValue('font-weight')}</span>
            </li>
            <li className="font-size">
              font-size:{' '}
              <span>{styles && styles.getPropertyValue('font-size')}</span>
            </li>
            <li className="font-line-height">
              line-height:{' '}
              <span>{styles && styles.getPropertyValue('line-height')}</span>
            </li>
            <li className="font-character-spacing">
              letter-spacing:{' '}
              <span>{styles && styles.getPropertyValue('letter-spacing')}</span>
            </li>
          </ul>
          <Button onClick={() => setFontEditIsOpen(true)}>Edit</Button>
        </Grid>
      </Grid>
    </>
  );
