...
export const StringCategorySelectable: React.FC<StringProps> = ({
  property,
  defaultValue,
  variant,
  description,
  ...
  const [title, setTitle] = useState<string>(
    property.getValue() || property.getDefaultValue() || defaultValue || 'None'
  );
    ...
    for (var i = 0; i < selectables.length; i++) {
      ...
      if (!variant || variant === 'dropdown') {
        if (s.display === 'None' && (!s.members || s.members.length === 0)) {
          ...
          r.push(
            <ListSubheader key={'label-' + s} inset={true}>
              {s.display}
            </ListSubheader>
          );
