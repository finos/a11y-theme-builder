...
export const ColorShadeCss: React.FC<Props> = ({
    className,
    name,
    id,
    lm,
    ...
    useEffect(() => {
        ...
        setColorValue(
            cv.replace('rgb(', '').replace('rgba(', '').replace(')', '')
        );
    ...
    return (
        ...
        <div className={className ?? ""} style={isBaseColor ? {paddingTop:"2px",paddingLeft:"4px",borderRadius:"0px",border:"1px",borderStyle:"solid",borderColor:"#CFD3D8"} : {}}>
            ...
            {isBaseColor && (
                <div
                    className="inserted-item text-center "
                    id="color-id"
                    style={{
                        fontSize: 'var(--caption-boldFontSize)',
                        fontWeight: 'var(--caption-boldFontWeight)',
                    }}
                >
                    Base
                </div>
