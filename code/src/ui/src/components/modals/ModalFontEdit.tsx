...
const weightUnsupportedAlert = (
  <Alert severity="warning" sx={{ width: 300, marginTop: 2 }}>
    The font weight provided is not supported by the selected font.
  </Alert>
);

const ModalFontEdit: React.FC<Props> = ({
  isOpen,
  onCancel,
  designSystem,
  textKey,
  ...
  if (isHeader) {
    defaultWeight =
      designSystem.atoms.displayAndHeaderStyles.headingDisplayFontWeight.getValue() ||
      400;
  } else {
    defaultWeight =
      designSystem.atoms.fontsSettings.fontWeights[1].getValue() || 500;
  }

  const primaryFont =
    designSystem.atoms.fontsSettings.primaryFont.getValue() || 'Open Sans';
  const secondaryFont =
    designSystem.atoms.fontsSettings.secondaryFont.getValue() || 'Open Sans';
  ...
  const [fontFamily, setFontFamily] = useState<string>(
    '' + (fontFamilyProperty.getValue() || defaultFont)
  );
  const [fontSize, setFontSize] = useState<number>(
    fontSizeProperty.getValue() || 16
  );
  const [fontWeight, setFontWeight] = useState<number>(
    fontWeightProperty.getValue() || defaultWeight
  );
  const [charSpacing, setCharSpacing] = useState<number>(
    charSpacingProperty.getValue() || 0
  );

  const [fontFamilyWithPrefix, setFontFamilyWithPrefix] = useState<string>(
    fontFamily === primaryFont ? primaryFontWithPrefix : secondaryFontWithPrefix
  );

  const [fontUncommon, setFontUncommon] = useState<boolean>(
    !FontWeightsUtil.isFontCommon(fontFamily)
  );
  ...
  const renderFontFamilySelectables = () => {
    return (
      <FormControl sx={{ m: 2, minWidth: 350 }}>
        <InputLabel id="fontFamilyLabel">{fontFamilyProperty.name}</InputLabel>
        <Select
        ...
        >
          ...
          <MenuItem
            key={secondaryFontWithPrefix}
            value={secondaryFontWithPrefix}
          >
  ...
  const renderFontWeightSelectables = () => {
    ...
    return (
      <FormControl sx={{ m: 2, minWidth: 100 }}>
        <InputLabel id="fontWeightLabel">{fontWeightProperty.name}</InputLabel>
        <Select
      ...
      <div className="modal modal-fontEdit">
        <div className="modal-content">
          <HeadingSection
            ...
            heading={'Editing ' + typographyStyling.name}
          />
          <ExampleSection>
          ...
          <SettingsSection>
            <form>
              ...
              {!fontUncommon || fontNotCommonAlert}
              {fontUncommon || (
                <Alert severity="info" sx={alertStyles}>
                  ...
                  <br />
                  <b>
                    {FontWeightsUtil.getFontWeights(fontFamily)?.join(', ')}
                  </b>
                  .
                </Alert>
              )}
              ...
              <FormControl sx={{ m: 2, minWidth: 500 }}>
                <label className="label-1">Font Size:</label>
              ...
              <br />
              <FormControl sx={{ m: 2, minWidth: 500 }}>
                <label className="label-1">Character Spacing:</label>
                <div className="caption">
                ...
                {isHeader || charSpacing > 0.12 * fontSize || (
                  <Alert severity="error">
                    WCAG AA guidelines require Line Height to be at least 0.12
                    times the font size.
                  </Alert>
                )}
              </FormControl>
          ...
          <div className="modal-footer">
            <div className="button-area">
              <Button
                onClick={handleSave}
                disabled={!isHeader && !(charSpacing > 0.12 * fontSize)}
              >
                Save
              </Button>
