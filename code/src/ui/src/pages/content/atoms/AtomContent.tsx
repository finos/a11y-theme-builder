...
const atomsList: { [key: string]: atomItem } = {
  ...
  subcolorThemes: {
    value: 'subcolorThemes',
    label: 'Sub Color Themes',
    atom: 'Subcolor Themes',
    disabled: true,
  },
  fontsSettings: {
    value: 'fontsSettings',
  ...
  displayAndHeaderStyles: {
    value: 'displayAndHeaderStyles',
    label: 'Display & Header Styles',
    atom: 'Display And Header Styles',
    disabled: true,
  },
  bodyStyles: {
    value: 'bodyStyles',
  ...
  smallTextStyles: {
    value: 'smallTextStyles',
    label: 'Small Text Styles',
    atom: 'Small Tests Styles',
    disabled: true,
  },
  statStyles: {
    value: 'statStyles',
  ...
  inputBackground: {
    ...
    atom: 'Input Background',
    disabled: true,
  },
  elevationSettings: {
    value: 'elevationSettings',
    label: 'Elevation Settings',
    atom: 'Elevation Settings',
    disabled: true,
  },
  bevelSettings: {
    value: 'bevelSettings',
    label: 'Bevel Settings',
    atom: 'Bevel Settings',
    disabled: true,
  },
  glowSettings: {
...
export const AtomContent: React.FC<Props> = ({ user, designSystem }) => {
  ...
  useEffect(() => {
    designSystem.setListener('AtomContent-isEditable', function (event: Event) {
  ...
  const [showAtom, setShowAtom] = useState(
    pref.get('atom-content-selected') || 'atoms'
  );
  useEffect(() => {
    pref.set('atom-content-selected', showAtom);
  }, [showAtom]);

  interface LeftNavAtomProps {
    atom: any;
    indent?: number;
    disabled?: boolean;
  }
  const LeftNavAtom: React.FC<LeftNavAtomProps> = ({
    atom,
    indent,
    disabled,
  }) => {
    ...
    <>
        ...
        <div className="design-system-editor-left-nav-scrollable">
          ...
          >
            ...
            <LeftNavItem
              text={'Shadow Atoms'}
              indent={1}
              onClick={() => setDisplayShadow(!displayShadow)}
            >
      ...
      <div className="design-system-editor-right-content">
        <div className="design-system-editor-right-content-scrollable">
          {showAtom === 'atoms' && <AtomicIntro changeTab={setShowAtom} />}
          {showAtom === atoms.colorPalette.value && (
            <ErrorHandler>
              <ColorPaletteAtom
                atom={designSystem.atoms.colorPalette}
                defaultColor="#ffffff"
                changeTab={setShowAtom}
              />
            </ErrorHandler>
          )}
          ...
          {showAtom === atoms.subcolorThemes.value && <div>subColorThemes</div>}
          ...
          {showAtom === atoms.chartColors.value && <div>chartColors</div>}
          ...
          {showAtom === atoms.inputBackground.value && (
            <ErrorHandler>
              <InputBackgroundsAtom
                inputBackground={designSystem.atoms.inputBackground}
              />
            </ErrorHandler>
          )}
          {showAtom === atoms.elevationSettings.value && (
            <ElevationsAtom
              elevationSettings={designSystem.atoms.elevationSettings}
            />
          )}
