...
export const StatesAtom: React.FC<Props> = ({ atom }) => {
    ...
    const [success, setSuccess] = useState([
        getCssValue(`--success`),
        getCssValue(`--dm-success`),
    ]);
    const [warning, setWarning] = useState([
        getCssValue(`--warning`),
        getCssValue(`--dm-warning`),
    ]);
    const [danger, setDanger] = useState([
        getCssValue(`--danger`),
        getCssValue(`--dm-danger`),
    ]);
    ...
    return (
        <div className="color-palette-right-content">
            <HeadingSection item={atom} title="Theme">
                <p>
                    Select the colors that will be used to display informative
                    text for the user. For example, these colors could be used
                    to display banners, popups or toasts informing the user of
                    the results of some task or process.
                </p>
            </HeadingSection>
            <ExampleSection>
                The color values are calculated from the input colors under
                Settings. They will not necessarily be the same value that was
                selected.
                <div className="top40" />
                <div style={{ display: 'flex', gap: '40px' }}>
                    <LightModeSection>{renderExample(true)}</LightModeSection>
                ...
                <SettingsSection>
                    ...
                    <div style={{ display: 'flex', gap: '40px' }}>
                        ...
                        />
                        <ColorProperty
                            property={atom.success}
                            label="Success"
                        />
                        <ColorProperty
                            property={atom.warning}
                            label="Warning"
                        />
