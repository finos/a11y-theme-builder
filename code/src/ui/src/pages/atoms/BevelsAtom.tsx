...
export const BevelsAtom: React.FC<Props> = ({ bevelSettings }) => {
    const horizontalShadowLengthProperty =
        bevelSettings.standard.horizontalShadowLength;
    const verticalShadowLengthProperty =
        bevelSettings.standard.verticalShadowLength;
    ...
    const [horizontalShadowLength, setHorizontalShadowLength] =
        useState<number>(horizontalShadowLengthProperty.getValue() || 0);
    const [verticalShadowLength, setVerticalShadowLength] = useState<number>(
        verticalShadowLengthProperty.getValue() || 0
    );
    ...
    const [lightGlowOpacity, setLightGlowOpacity] = useState<number>(
        lightGlowOpacityProperty.getValue() || 0
    );
    const [darkShadowOpacity, setDarkShadowOpacity] = useState<number>(
        darkShadowOpacityProperty.getValue() || 0
    );
    const [percentChange, setPercentChange] = useState<number>(
        percentChangeProperty.getValue() || 0
    );

    async function handleHorizontalShadowLengthChange(
        event: any
    ): Promise<void> {
    ...
    const inverseHorizontalShadowLengthProperty =
        bevelSettings.inverse.horizontalShadowLength;
    const inverseVerticalShadowLengthProperty =
        bevelSettings.inverse.verticalShadowLength;
    ...
    const inverseLightGlowOpacityProperty =
        bevelSettings.inverse.lightGlowOpacity;
    const inverseDarkShadowOpacityProperty =
        bevelSettings.inverse.darkShadowOpacity;
    const inversePercentChangeProperty = bevelSettings.inverse.percentageChange;

    const [inverseHorizontalShadowLength, setInverseHorizontalShadowLength] =
        useState<number>(inverseHorizontalShadowLengthProperty.getValue() || 0);
    const [inverseVerticalShadowLength, setInverseVerticalShadowLength] =
        useState<number>(inverseVerticalShadowLengthProperty.getValue() || 0);
    const [inverseBlurRadius, setInverseBlurRadius] = useState<number>(
        inverseBlurRadiusProperty.getValue() || 0
    );
    const [inverseSpreadRadius, setInverseSpreadRadius] = useState<number>(
        inverseSpreadRadiusProperty.getValue() || 0
    );
    const [inverseLightGlowOpacity, setInverseLightGlowOpacity] =
        useState<number>(inverseLightGlowOpacityProperty.getValue() || 0);
    const [inverseDarkShadowOpacity, setInverseDarkShadowOpacity] =
        useState<number>(inverseDarkShadowOpacityProperty.getValue() || 0);
    const [inversePercentChange, setInversePercentChange] = useState<number>(
        inversePercentChangeProperty.getValue() || 0
    );

    async function handleInverseHorizontalShadowLengthChange(
        event: any
    ): Promise<void> {
        const value = Number(event.target.value);
    ...
    async function handleInverseVerticalShadowLengthChange(
        event: any
    ): Promise<void> {
        const value = Number(event.target.value);
    ...
    async function handleInverseLightGlowOpacityChange(
        event: any
    ): Promise<void> {
        const value = Number(event.target.value);
    ...
    async function handleInverseDarkShadowOpacityChange(
        event: any
    ): Promise<void> {
        const value = Number(event.target.value);
    ...
    return (
        <div>
            <HeadingSection
                item={bevelSettings}
                title="Bevels & Inverse Bevels"
            >
                Bevels add 3D depth to a graphic or text object by making its
                edges appear sloped.
            </HeadingSection>
            <ExampleSection title="Bevel Shadow Settings">
                <div className="row">
                    <div className="col-6">
                        ...
                        <div className="form-row">
                            <label className="label-1">Dark Glow Opacity</label>
                            <Slider
            ...
            <ExampleSection title="Inverse Bevel Shadow Settings">
                <div className="row">
                    ...
                    <div className="col-6">
                        ...
                        <ExampleBevel
                            className="bevel-1-inverse"
                            label="Bevel 1"
                        />
                        <ExampleBevel
                            className="bevel-2-inverse"
                            label="Bevel 2"
                        />
                        <ExampleBevel
                            className="bevel-3-inverse"
                            label="Bevel 3"
                        />
                        <ExampleBevel
                            className="bevel-4-inverse"
                            label="Bevel 4"
                        />
                        <ExampleBevel
                            className="bevel-5-inverse"
                            label="Bevel 5"
                        />
                        <ExampleBevel
                            className="bevel-6-inverse"
                            label="Bevel 6"
                        />
                        <ExampleBevel
                            className="bevel-7-inverse"
                            label="Bevel 7"
                        />
                        <ExampleBevel
                            className="bevel-8-inverse"
                            label="Bevel 8"
                        />
                        <ExampleBevel
                            className="bevel-9-inverse"
                            label="Bevel 9"
                        />
                    </div>
                </div>
                ...
                <div className="row top40">
                    ...
                    <div className="col-6">
                        ...
                        <ExampleBevel
                            className="recessed-1"
                            label="Recessed 1"
                        />
                        <ExampleBevel
                            className="recessed-2"
                            label="Recessed 2"
                        />
                        <ExampleBevel
                            className="recessed-3"
                            label="Recessed 3"
                        />
                        <ExampleBevel
                            className="recessed-4"
                            label="Recessed 4"
                        />
                        <ExampleBevel
                            className="recessed-5"
                            label="Recessed 5"
                        />
                        <ExampleBevel
                            className="recessed-6"
                            label="Recessed 6"
                        />
                        <ExampleBevel
                            className="recessed-7"
                            label="Recessed 7"
                        />
                        <ExampleBevel
                            className="recessed-8"
                            label="Recessed 8"
                        />
                        <ExampleBevel
                            className="recessed-9"
                            label="Recessed 9"
                        />
                    </div>
                </div>
